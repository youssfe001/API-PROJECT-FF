...
const { getCredentialForRegion } = require("./credentials");
...
async function getGarenaToken(uid, password) {
  ...
  console.log(`[auth] Garena OK open_id=${(resp.body.open_id || "").slice(0, 8)}...`);
...
async function getMajorLogin(access_token, open_id, region) {
  ...
  const reqBuf    = encodeProto(
    {
      ...
      // Client version — must match real client
      clientversion:  "1.122.26",
      platid:         1,           // 1 = Android

      // Device fingerprint (real Samsung Galaxy A34 5G values)
      systemsoftware: "Android OS 13 / API-33 (TP1A.220624.014/A346BXXS3CWL1)",
      systemhardware: "samsung SM-A346B",
      devicemodel:    "SM-A346B",
      devicetype:     "android",
      cpuhardware:    "MT6877V/TNZA",
      memory:         6,             // 6 GB RAM
      screenwidth:    1080,
      screenhight:    2340,
      dpi:            "390",
      glrender:       "Mali-G57 MC2",
      glversion:      "OpenGL ES 3.2 v1.r40p0-01eac0.a40ce03e3f1ffed3e4ade92e9e9e5b2e",
      network:        "WIFI",
      language:       "en",
      region:         region,

      // Anti-emulator: explicitly declare not emulator
      isemulator:     false,
      signaturemd5:   "7428b253defc164018c604a1ebbfebdf",
    },
    "MajorLogin.request"
  );
  ...
  console.log(`[auth] MajorLogin → host=${hostname} encLen=${encrypted.length}`);
  ...
  console.log(`[auth] MajorLogin ← status=${resp.status} bodyLen=${resp.raw.length}`);
...
async function getJwtToken(region) {
  ...
  const acct = selected.credential;

  console.log(`[auth] login region=${region} uid=${acct.uid} source=${selected.source}`);
