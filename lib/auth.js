...
async function getMajorLogin(access_token, open_id, region) {
  ...
  // الرد مشفَّر بـ AES — نفك التشفير أولاً ثم نُفسِّر الـ protobuf
  const decryptedHex = decryptAes(resp.raw.toString("hex"));
  const decoded = decodeProto(Buffer.from(decryptedHex, "hex"), "MajorLogin.response");

  if (!decoded.token) {
    throw new Error(`getMajorLogin no token: ${JSON.stringify(decoded)}`);
  }
