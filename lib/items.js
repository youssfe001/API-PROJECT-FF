...
function mergeItem(id, base, enriched) {
  ...
  return {
    ...
    ffIcon: enriched?.ff || null,
    ffIconKind: enriched?.ffk || null,
    proxyIconPath: `/api/items/icon/${encodeURIComponent(strId)}.png`,
...
function resolveItems(ids) {
  ...
  return ids.map((id) => {
    ...
    return {
      ...
      ffIcon: item?.ffIcon || null,
      ffIconKind: item?.ffIconKind || null,
