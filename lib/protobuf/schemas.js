...
// ─── GetRankingList.proto ─────────────────────────────────────────────────────
// Field mapping verified from actual binary response:
//   field 1  (uint64)  = accountid
//   field 2  (uint32)  = headpic (avatar ID)
//   field 3  (uint32)  = rankingpoints (BR score)
//   field 4  (uint32)  = csrankingpoints
//   field 5  (uint32)  = rankpos (leaderboard position)
//   field 6  (uint32)  = level
//   field 12 (uint32)  = bannerid
//   field 15 (uint32)  = role
//   field 16 (uint32)  = haselitepass (treated as uint)
//   field 23 (string)  = nickname  ← verified: LEN-delimited in raw hex

const RankingEntry = {
  accountid:        { field: 1,  type: "uint64" },
  headpic:          { field: 2,  type: "uint32" },
  rankingpoints:    { field: 3,  type: "uint32" },
  csrankingpoints:  { field: 4,  type: "uint32" },
  rankpos:          { field: 5,  type: "uint32" },
  level:            { field: 6,  type: "uint32" },
  bannerid:         { field: 12, type: "uint32" },
  role:             { field: 15, type: "uint32" },
  haselitepass:     { field: 16, type: "uint32" },
  nickname:         { field: 23, type: "string" },
};

const GetRankingList_request = {
  type: { field: 1, type: "uint32" }, // 1 = BR, 2 = CS
  page: { field: 2, type: "uint32" }, // 0-based page
};

// Outer response:
//   field 1  (repeated message) = entries
//   field 2  (fixed64)          = server timestamp / metadata (skip)
const GetRankingList_response = {
  entries: { field: 1, type: "message", messageType: "RankingEntry", repeated: true },
};
...
module.exports = {
  ...
  "GetRankingList.request":           GetRankingList_request,
  ...
  "GetRankingList.response":       GetRankingList_response,
