...
2. Set Runtime to Node.js (auto via `@vercel/node`).
3. Add one of these environment variables in Vercel Project Settings:
   - `FF_GUESTS_JSON` (recommended)
   - or `FF_ACCOUNTS_JSON` (fallback)

Example value for `FF_GUESTS_JSON`:

```json
{"IND":[{"uid":"1234567890","password":"guest_password_here"}]}
```
...
curl "https://<your-domain>/api/guest-generator?region=IND"
```

### Notes

- On cloud deploys, `config/guests.json` is usually not present because it is git-ignored.
- Environment JSON (`FF_GUESTS_JSON`) is now supported directly by the credential loader.
